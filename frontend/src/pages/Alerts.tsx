...
import {ALL_COLUMNS,loadColumns,loadViews,saveColumns,saveCustomView,type ColumnKey,type SavedView} from '../services/preferences';

export function Alerts({alerts,onSelect}:{alerts:Alert[];onSelect:(a:Alert)=>void}){
  const[q,setQ]=useState(''),[severity,setSeverity]=useState('all'),[columns,setColumns]=useState<ColumnKey[]>(loadColumns),[sort,setSort]=useState<'newest'|'oldest'>('newest');
  ...
  }).sort((a,b)=>(sort==='newest'?-1:1)*(new Date(a.timestamp).getTime()-new Date(b.timestamp).getTime())),[alerts,q,severity,predicate,sort]);
  ...
  return <><header className="page-head"><div><span className="eyebrow">DETECTION MANAGEMENT</span><h1>Detection queue</h1><p>Prioritize, investigate, and disposition model-generated behavioral findings.</p></div><div className="page-controls popover-anchor"><button onClick={()=>{setColumnsOpen(!columnsOpen);setViewsOpen(false)}}><SlidersHorizontal/> Edit columns</button><button onClick={()=>{setViewsOpen(!viewsOpen);setColumnsOpen(false)}}><Filter/> Saved views</button>
    ...
    <section className="panel alerts full"><div className="queue-count"><b>{shown.length}</b> detections <button onClick={()=>setSort(sort==='newest'?'oldest':'newest')}>Sorted by {sort} first ↕</button></div><AlertTable alerts={shown} onSelect={onSelect} columns={columns}/></section></>;
}
