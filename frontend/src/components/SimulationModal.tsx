...
const scenarios=['mixed','brute_force','credential_misuse','lateral_movement','impossible_travel','device_spoofing','cold_start','concept_drift'];
...
export function SimulationModal({onClose}:{onClose:()=>void}){
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={event=>event.target===event.currentTarget&&onClose()}><section className="simulation-modal" role="dialog" aria-modal="true" aria-labelledby="simulation-title">
    <header><div><span className="eyebrow">SYNTHETIC REAL-TIME SECURITY EVENTS</span><h2 id="simulation-title">Run security simulation</h2><p>Every generated event is validated, persisted, feature-engineered, scored by both models, explained, and broadcast live.</p></div><button className="icon-button" onClick={onClose} aria-label="Close simulation"><X/></button></header>
    <div className="simulation-form"><label>Scenario<select value={scenario} disabled={running} onChange={event=>{const value=event.target.value;setScenario(value);if(value==='concept_drift'&&count<40)setCount(45)}}>{scenarios.map(value=><option key={value} value={value}>{value.replaceAll('_',' ')}</option>)}</select></label>
      ...
      <label>Event count<input type="number" min={scenario==='concept_drift'?40:1} max={500} disabled={running} value={count} onChange={event=>setCount(Math.max(scenario==='concept_drift'?40:1,Math.min(500,Number(event.target.value))))}/></label></div>
