...
def make_bundle():
    rng=np.random.default_rng(42);x=rng.normal(size=(100,24));y=np.array(["normal"]*70+["brute_force"]*15+["device_spoofing"]*15)
    ...
    sequence=GRUSequenceDetector(24,random_state=42).fit(scaled,y,np.array([f"e{i%10}" for i in range(100)]))
    return ModelBundle("test","2.0.0",FeaturePipeline.names,scaler,anomaly,sequence,classifier,50,{})
...
def test_model_save_load_and_inference(tmp_path:Path):
    ...
    loaded.validate(FeaturePipeline.names);result=loaded.infer(np.zeros(24));assert 0<=result["anomaly_score"]<=1
...
def test_risk_bounds():
    ...
    result=RiskService().score(inference,np.full(24,100),event(resource_sensitivity=1,is_privileged_action=True),.1)
