...
class GRUSequenceDetector:
    ...
    def _error(self, predicted: np.ndarray, observed: np.ndarray) -> np.ndarray:
        """Focus reconstruction loss on the strongest sparse deviations."""
        absolute = np.abs(np.asarray(predicted) - np.asarray(observed))
        width = absolute.shape[-1]; top_k = min(max(self.error_top_k, 1), width)
    ...
    def fit(self, scaled: np.ndarray, labels: np.ndarray, entities: np.ndarray):
        ...
        predictions = design @ self.decoder; errors = self._error(predictions, target)
        self.error_low, self.error_high = float(np.quantile(errors, .5)), float(np.quantile(errors, .9975))
        return self
