...
async def ingest(event: AccessEvent, db: Session = Depends(get_db)):
    result = score_event(event, db); await event_bus.publish({"type": "scored_event", "data": result}); return result


@router.post("/batch", response_model=list[PredictionResponse])
async def batch(payload: EventBatch, db: Session = Depends(get_db)):
    if len(payload.events) > get_settings().max_batch_size: raise HTTPException(413, "batch is too large")
    results = []
    for event in payload.events:
        result = score_event(event, db); results.append(result); await event_bus.publish({"type": "scored_event", "data": result})
    return results
