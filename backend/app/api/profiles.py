...
def display_name(user_id: str) -> str:
    suffix = user_id.removeprefix("usr-").replace("-", " ").title()
    return f"User {suffix}" if suffix else user_id
...
def users(search: str = "", limit: int = Query(20, ge=1, le=100), db: Session = Depends(get_db)):
    ...
    return [{"user_id": row.user_id, "display_name": display_name(row.user_id), "role": row.role,
...
def profile(user_id: str, db: Session = Depends(get_db)):
    ...
    return {"user_id": user_id, "display_name": display_name(user_id), "role": event.user_role,
            ...
            "cold_start_explanation": ("This user has insufficient trusted history. Peer or global behavior is being used as the baseline."
